...
const registerHandler = async (req, res) => {
    try {
        ...
        if (userExists) return res.status(409).json({ success: false, message: "User already exists" });

        const hashedPassword = await hashPassword(password);
        const user = await new userModel({ name, email, password: hashedPassword, phone, username, answer, role: 1 }).save();
...
const loginHandler = async (req, res) => {
    try {
        ...
        const token = jwt.sign({ _id: user._id, role: 1 }, JWT_SECRET, { expiresIn: "1d" });

        res.status(200).json({
            success: true,
            message: "Login successful",
            token,
            user: { _id: user._id, name: user.name, email: user.email, username: user.username, phone: user.phone, role: 1 },
        });
    } catch (err) {
        console.error(err);
        res.status(500).json({ success: false, message: "Server error", error: err.message });
...
const googleLoginHandler = async (req, res) => {
    try {
        ...
        if (!email_verified) return res.status(400).json({ success: false, message: "Google login failed" });
        ...
        res.status(200).json({
            ...
            token,
            user: { _id: user._id, name: user.name, email: user.email, username: user.username, role: 1 },
        });
    } catch (err) {
        console.error(err);
        res.status(500).json({ success: false, message: "Server error", error: err.message });
...
const adminLoginHandler = async (req, res) => {
    try {
        ...
        const token = jwt.sign({ _id: admin._id, role: 2 }, JWT_SECRET, { expiresIn: "1d" });
...
const forgotHandler = async (req, res) => {
  try {
    ...
    if (step === "request") {
      ...
      const expiresAt = new Date(Date.now() + parseInt(process.env.OTP_EXPIRES_MIN) * 60 * 1000);
    ...
    if (step === "reset") {
      ...
      const otpRecord = await Otp.findOne({ email, code: otp });
      if (!otpRecord) return res.status(400).json({ success: false, message: "OTP verification expired. Please request a new OTP" });
    ...
    // Invalid step
    return res.status(400).json({ success: false, message: "Invalid step parameter" });
  } catch (err) {
    console.error(err);
    res.status(500).json({ success: false, message: "Server error", error: err.message });
  }
...
const getTotalUsers = async (req, res) => {
    try {
        ...
        res.status(200).json({ success: true, message: "Users fetched successfully", total });
    } catch (err) {
        console.error(err);
        res.status(500).json({ success: false, message: "Server error", error: err.message });
    }
...
const adminGetAllUsers = async (req, res) => {
    try {
        ...
        const formattedUsers = users.map(user => ({
            ...
            name: user.name,
            email: user.email,
            role: user.role === 2 ? "Admin" : user.role === 1 ? "Developer" : "Viewer",
            status: "active",
    ...
    try {
        ...
        const user = await userModel.findByIdAndUpdate(
            userId,
            {
                ...
                role: role === "Admin" ? 2 : role === "Developer" ? 1 : 1,
            },
            { new: true }
        ).select("-password");
